...
export const addMenuItem=async(req,res)=>{
   try {
      // debug logging
      console.log("addMenuItem body", req.body);
      console.log("addMenuItem file", req.file);
     ...
     const newMenuItem=await Menu.create({
      ...
      menuItem: newMenuItem,
            ...
            console.error("addMenuItem error", error);
             // send back actual error message for easier debugging
...
export const getAllMenuItems=async(req,res)=>{
   try {
      const menuItems=await Menu.find().populate("category","name mainType").sort({createdAt:-1});
